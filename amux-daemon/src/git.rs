...
impl GitOps {
    ...
    ///
    /// Worktrees are stored in `~/.amux/repos/{repo_name}/{branch}`.
    pub fn create_worktree(
        ...
        // Worktree path: ~/.amux/repos/{repo_name}/{branch}
        let wt_name = branch.replace('/', "-");
        let repo_name = base_path
            .file_name()
            .and_then(|n| n.to_str())
            .unwrap_or("unknown");

        let amux_dir = dirs::home_dir()
            .ok_or_else(|| GitError::Custom("Cannot find home directory".to_string()))?
            .join(".amux")
            .join("repos")
            .join(repo_name);
        ...
        let wt_path = amux_dir.join(&wt_name);
