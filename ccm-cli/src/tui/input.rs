...
pub fn handle_input_sync(app: &mut App, key: KeyEvent) -> Option<AsyncAction> {
    ...
    if app.input_mode == InputMode::TodoPopup {
        return handle_todo_popup_sync(app, key);
    }

    if matches!(app.input_mode, InputMode::AddTodo { .. }) {
        return handle_add_todo_mode_sync(app, key);
    }

    if matches!(app.input_mode, InputMode::EditTodo { .. }) {
        return handle_edit_todo_mode_sync(app, key);
    }

    if matches!(app.input_mode, InputMode::EditTodoDescription { .. }) {
        return handle_edit_todo_description_mode_sync(app, key);
    }

    if matches!(app.input_mode, InputMode::ConfirmDeleteTodo { .. }) {
        return handle_confirm_delete_todo_sync(app, key);
    }
...
fn handle_todo_popup_sync(app: &mut App, key: KeyEvent) -> Option<AsyncAction> {
    match key.code {
...
fn handle_add_todo_mode_sync(app: &mut App, key: KeyEvent) -> Option<AsyncAction> {
    match key.code {
        KeyCode::Esc => {
            app.input_mode = InputMode::TodoPopup;
            app.input_buffer.clear();
...
fn handle_edit_todo_mode_sync(app: &mut App, key: KeyEvent) -> Option<AsyncAction> {
    match key.code {
        KeyCode::Esc => {
            app.input_mode = InputMode::TodoPopup;
            app.input_buffer.clear();
...
fn handle_edit_todo_description_mode_sync(app: &mut App, key: KeyEvent) -> Option<AsyncAction> {
    match key.code {
        KeyCode::Esc => {
            app.input_mode = InputMode::TodoPopup;
            app.input_buffer.clear();
...
fn handle_confirm_delete_todo_sync(app: &mut App, key: KeyEvent) -> Option<AsyncAction> {
    match key.code {
